...
.ui-select__trigger { width: 100%; min-height: 48px; display: flex; align-items: center; gap: 7px; padding: 8px 10px 8px 12px; border: 1px solid var(--color-border); border-radius: 11px; background: var(--color-surface); color: var(--color-text-primary); font: inherit; font-size: 14px; text-align: left; cursor: pointer; transition: border-color .15s, box-shadow .15s, background .15s; }
.ui-select__trigger:hover { border-color: var(--color-border-strong); }.ui-select--open .ui-select__trigger,.ui-select__trigger:focus-visible { border-color: var(--color-focus); box-shadow: 0 0 0 3px var(--color-focus-ring); outline: 0; }
.ui-select__value { min-width: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.ui-select__value--placeholder { color: var(--color-text-muted); }
.ui-select__leading { width: 21px; height: 21px; flex: 0 0 21px; display: grid; place-items: center; border-radius: 50%; background: var(--color-primary-soft); color: var(--color-primary-hover); font-size: 9px; }
.ui-select__chevron { width: 14px; height: 14px; flex: 0 0 14px; fill: none; stroke: currentColor; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; color: var(--color-text-muted); transition: transform .15s; }.ui-select--open .ui-select__chevron { transform: rotate(180deg); }
.ui-select--disabled { opacity: .55; }.ui-select--disabled .ui-select__trigger { cursor: default; }
</style>

<style>
.ui-select__menu { z-index: 900; overflow-y: auto; padding: 5px; border: 1px solid var(--color-border); border-radius: 13px; background: var(--color-surface-raised); box-shadow: var(--shadow-medium); overscroll-behavior: contain; }
.ui-select__option { width: 100%; min-height: 48px; display: flex; align-items: center; gap: 8px; padding: 7px 9px; border: 0; border-radius: 9px; background: transparent; color: var(--color-text-secondary); font-family: var(--font); font-size: 14px; font-weight: 650; text-align: left; cursor: pointer; }.ui-select__option:hover,.ui-select__option.active { background: var(--color-surface-hover); color: var(--color-text-primary); }.ui-select__option.selected { background: var(--color-primary-soft); color: var(--color-primary-hover); }.ui-select__option > span:nth-child(2),.ui-select__option > span:first-child:last-of-type { min-width: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.ui-select__option > svg { width: 14px; height: 14px; flex: 0 0 14px; fill: none; stroke: var(--color-primary-hover); stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
.ui-select__option-icon { width: 22px; height: 22px; flex: 0 0 22px; display: grid; place-items: center; border-radius: 50%; background: var(--color-surface); color: var(--color-primary-hover); box-shadow: inset 0 0 0 1px var(--color-border); font-size: 10px; }
