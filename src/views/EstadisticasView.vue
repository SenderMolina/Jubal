...
.progress-hero { position: relative; overflow: hidden; padding: 21px 19px 18px; border: 1px solid var(--color-border); border-radius: 24px; color: var(--color-text-primary); background: var(--color-surface); box-shadow: var(--shadow-small); }
.progress-hero::after { content: '♫'; position: absolute; right: 19px; top: 10px; color: var(--color-primary-soft); font-size: 88px; transform: rotate(10deg); }
.progress-hero__eyebrow, .progress-card__head span { font-size: 9px; font-weight: 900; letter-spacing: .1em; text-transform: uppercase; color: var(--color-primary); }
.progress-hero__main { position: relative; z-index: 1; display: flex; align-items: center; gap: 13px; margin-top: 10px; }.progress-hero__icon { width: 58px; height: 58px; flex: 0 0 58px; border: 2px solid var(--color-primary); border-radius: 18px; display: grid; place-items: center; color: var(--color-primary); background: var(--color-primary-soft); font-size: 25px; box-shadow: none; }.progress-hero h2 { font-size: 18px; }.progress-hero p { margin-top: 3px; color: var(--color-text-secondary); font-size: 10px; }
.stat-powers { display: grid; grid-template-columns: repeat(3,1fr); gap: 8px; }.stat-powers article { min-width: 0; padding: 13px 6px; border: 1px solid var(--color-border); border-radius: 17px; background: var(--color-surface); text-align: center; box-shadow: var(--shadow-small); }.stat-powers__icon { display: block; font-size: 17px; }.stat-powers strong { display: block; margin-top: 3px; font-size: 17px; font-variant-numeric: tabular-nums; }.stat-powers small { display: block; margin-top: 1px; color: var(--color-text-muted); font-size: 8px; }
.progress-card { padding: 16px; border: 1px solid var(--color-border); border-radius: 20px; background: var(--color-surface); box-shadow: var(--shadow-small); }.progress-card__head { display: flex; align-items: flex-end; justify-content: space-between; gap: 10px; margin-bottom: 15px; }.progress-card__head span { color: var(--color-primary); }.progress-card__head h3 { margin-top: 3px; font-size: 14px; }.progress-card__head > strong { color: var(--color-primary-hover); font-size: 13px; }.progress-card__head a { color: var(--color-link); font-size: 10px; font-weight: 800; text-decoration: none; }
.week-chart { height: 126px; display: grid; grid-template-columns: repeat(7,1fr); gap: 7px; padding-top: 8px; border-bottom: 1px solid var(--color-border); }.week-chart__day { min-width: 0; display: flex; flex-direction: column; align-items: center; justify-content: flex-end; gap: 6px; }.week-chart__column { width: 100%; height: 91px; display: flex; align-items: flex-end; justify-content: center; }.week-chart__column > span { position: relative; display: block; width: min(22px, 72%); min-height: 3px; border-radius: 7px 7px 2px 2px; background: var(--chart-2); transition: height .65s cubic-bezier(.2,.8,.2,1); }.week-chart__column i { position: absolute; top: -14px; left: 50%; transform: translateX(-50%); color: var(--color-text-muted); font-size: 7px; font-style: normal; }.week-chart__day small { padding-bottom: 5px; color: var(--color-text-muted); font-size: 8px; font-weight: 700; text-transform: uppercase; }.week-chart__day.today small { color: var(--color-primary); }.week-chart__day.today .week-chart__column > span { background: var(--chart-1); }
.weekly-goal { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 8px; margin-top: 12px; font-size: 9px; color: var(--color-text-secondary); }.weekly-goal b { color: var(--color-text-primary); }.weekly-goal > div { display: flex; gap: 3px; }.weekly-goal i { height: 5px; flex: 1; border-radius: 99px; background: var(--color-surface-secondary); }.weekly-goal i.filled { background: var(--color-secondary); }.weekly-goal strong { color: var(--color-primary-hover); font-size: 8px; }
.achievement-list { display: flex; flex-direction: column; gap: 8px; }.achievement-list article { display: flex; align-items: center; gap: 11px; padding: 10px; border: 1px solid var(--color-border); border-radius: 14px; background: var(--color-surface-secondary); opacity: .55; }.achievement-list article.unlocked { border-color: rgba(var(--color-accent-rgb),.24); background: var(--color-accent-soft); opacity: 1; }.achievement-list article > span { width: 35px; height: 35px; flex: 0 0 35px; display: grid; place-items: center; border-radius: 11px; background: var(--color-surface); font-size: 17px; filter: grayscale(1); }.achievement-list article.unlocked > span { filter: none; box-shadow: 0 3px 9px rgba(var(--color-accent-rgb),.12); }.achievement-list article div { min-width: 0; flex: 1; display: flex; flex-direction: column; }.achievement-list article strong { font-size: 11px; }.achievement-list article small { margin-top: 2px; color: var(--color-text-muted); font-size: 8px; }.achievement-list article > b { color: var(--color-success); font-size: 11px; }
.skills-summary { display: flex; align-items: center; gap: 22px; padding: 3px 3px 5px; }.skills-ring { --degree: calc(var(--skill-progress) * 3.6deg); width: 90px; height: 90px; flex: 0 0 90px; position: relative; display: flex; flex-direction: column; align-items: center; justify-content: center; border-radius: 50%; background: conic-gradient(var(--color-secondary) var(--degree), var(--color-surface-secondary) 0); }.skills-ring::after { content: ''; position: absolute; inset: 8px; border-radius: 50%; background: var(--color-surface); }.skills-ring strong,.skills-ring small { position: relative; z-index: 1; }.skills-ring strong { font-size: 18px; }.skills-ring small { color: var(--color-text-muted); font-size: 8px; }.skills-summary__legend { flex: 1; display: flex; flex-direction: column; gap: 10px; }.skills-summary__legend span { display: flex; align-items: center; gap: 7px; color: var(--color-text-secondary); font-size: 9px; }.skills-summary__legend i { width: 8px; height: 8px; border-radius: 3px; background: var(--color-border); }.skills-summary__legend i.mastered { background: var(--color-success); }.skills-summary__legend i.practicing { background: var(--color-accent); }.skills-summary__legend b { margin-left: auto; color: var(--color-text-primary); }
.skills-ring.complete { background: conic-gradient(var(--color-success) var(--degree), var(--color-surface-secondary) 0); }
.stat-powers--records article { box-shadow: none; background: var(--color-surface-secondary); border: 0; }
.heatmap { display: grid; grid-auto-flow: column; grid-template-rows: repeat(7, 1fr); gap: 3px; }
.heatmap i, .heatmap-legend i { display: block; aspect-ratio: 1; border-radius: 3px; background: var(--color-surface-secondary); }
.heatmap--l1 { background: color-mix(in srgb, var(--color-primary) 30%, var(--color-surface-secondary)) !important; }
.heatmap--l2 { background: color-mix(in srgb, var(--color-primary) 55%, var(--color-surface-secondary)) !important; }
.heatmap--l3 { background: color-mix(in srgb, var(--color-primary) 78%, var(--color-surface-secondary)) !important; }
.heatmap--l4 { background: var(--color-primary) !important; }
...
.heatmap-legend small { color: var(--color-text-muted); font-size: 8px; margin: 0 3px; }
.attention-review { color: var(--color-primary-hover) !important; font-size: 9px !important; font-weight: 800; text-transform: uppercase; letter-spacing: .06em; }
.progress-empty,.progress-hint { color: var(--color-text-muted); font-size: 11px; line-height: 1.5; }.progress-hint { padding: 4px 8px; text-align: center; }
.practice-ranking,.attention-list { display: flex; flex-direction: column; gap: 7px; }.practice-ranking a,.attention-list a { display: flex; align-items: center; gap: 10px; padding: 10px; border: 1px solid var(--color-border); border-radius: 13px; background: var(--color-surface-secondary); color: var(--color-text-primary); text-decoration: none; }.practice-ranking__body,.attention-list a > span { min-width: 0; flex: 1; display: flex; flex-direction: column; }.practice-ranking strong,.attention-list strong { overflow: hidden; font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.practice-ranking small,.attention-list small { margin-top: 2px; color: var(--color-text-muted); font-size: 8px; }.practice-ranking__tempo { display: flex; flex-direction: column; align-items: flex-end; color: var(--color-primary-hover); }.practice-ranking__tempo b { font-size: 10px; }.practice-ranking__tempo i { color: var(--color-success); font-style: normal; font-weight: 800; }.practice-ranking__tempo i.down { color: var(--color-danger); }.attention-list a > b { min-width: 42px; color: var(--color-accent-text); font-size: 13px; text-align: right; }
...
@media (prefers-reduced-motion: reduce) { .week-chart__column > span { transition: none; } }

.progress-hero { border:1px solid var(--color-border);border-radius:28px;background:var(--color-surface);box-shadow:var(--shadow-small); }.progress-hero__eyebrow,.progress-card__head span { font-size:11px; }.progress-hero h2 { font-size:22px; }.progress-hero p { font-size:13px; }.stat-powers article { min-height:82px;border-radius:19px;box-shadow:none; }.stat-powers strong { color:var(--color-primary);font-size:20px; }.stat-powers small { font-size:11px; }
.progress-card { padding:18px;border-radius:24px;box-shadow:var(--shadow-small); }.progress-card__head h3 { font-size:17px; }.progress-card__head>strong,.progress-card__head a { font-size:13px; }.week-chart__column i,.week-chart__day small { font-size:10px; }.weekly-goal { font-size:12px; }.weekly-goal strong { font-size:11px; }
